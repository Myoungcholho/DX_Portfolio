...
void CubeMapSkyRenderer::Initialize()
{
	...
	//// .dds 파일 읽기
	//D3D::Get()->CreateDDSTexture(EnvHDR_Name.c_str(), true, cubeMapping.envSRV);
	//D3D::Get()->CreateDDSTexture(Specular_Name.c_str(), true, cubeMapping.specularSRV);
	//D3D::Get()->CreateDDSTexture(DiffuseHDR_Name.c_str(), true, cubeMapping.irradianceSRV);
	//D3D::Get()->CreateDDSTexture(Brdf_Name.c_str(), false, cubeMapping.brdfSRV);

	//// IBL사용하기 위해 저장소에 등록
	//TextureManager* textureManager =  Engine::Get()->GetTextureManager();
	//textureManager->Register(L"SkyBox", cubeMapping.envSRV.Get());
	//textureManager->Register(L"SkyBox_Diffuse", cubeMapping.irradianceSRV.Get());
	//textureManager->Register(L"SkyBox_Specular", cubeMapping.specularSRV.Get());
	//textureManager->Register(L"SkyBox_BRDF", cubeMapping.brdfSRV.Get());

	//cubeMapping.CubeMesh = std::make_shared<FMesh>();
	...
	//// VS의 ConstantBuffer
	//D3D::Get()->CreateConstantBuffer(worldInvConstantBufferData, cubeMapping.CubeMesh->VSCBuffer);
	...
	//D3D::Get()->CreateVertexBuffer(cubeMeshData.vertices, cubeMapping.CubeMesh->VertexBuffer);

	//cubeMapping.CubeMesh->IndexCount = UINT(cubeMeshData.indices.size());
	//D3D::Get()->CreateIndexBuffer(cubeMeshData.indices, cubeMapping.CubeMesh->IndexBuffer);
	...
	//	basicInputElements,
	//	cubeMapping.VertexShader,
	//	cubeMapping.InputLayout
	//);
	//D3D::Get()->CreatePixelShader(L"CubeMapping/CubeMappingPS.hlsl",cubeMapping.PixelShader);
}
...
void CubeMapSkyRenderer::Tick()
{
	...
	//D3D::Get()->UpdateBuffer(worldInvConstantBufferData, cubeMapping.CubeMesh->VSCBuffer);
...
void CubeMapSkyRenderer::Render(const bool& mirror)
{
	...
	//// IA
	//D3D::Get()->GetDeviceContext()->IASetInputLayout(cubeMapping.InputLayout.Get());
	//D3D::Get()->GetDeviceContext()->IASetVertexBuffers(0, 1, cubeMapping.CubeMesh->VertexBuffer.GetAddressOf(), &stride, &offset);
	//D3D::Get()->GetDeviceContext()->IASetIndexBuffer(cubeMapping.CubeMesh->IndexBuffer.Get(), DXGI_FORMAT_R32_UINT, 0);
	//D3D::Get()->GetDeviceContext()->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);

	//// VS
	//D3D::Get()->GetDeviceContext()->VSSetConstantBuffers(0, 1, cubeMapping.CubeMesh->VSCBuffer.GetAddressOf());
	//if(mirror == false)
	...
	//D3D::Get()->GetDeviceContext()->VSSetShader(cubeMapping.VertexShader.Get(), 0, 0);

	//// PS
	//D3D::Get()->GetDeviceContext()->PSSetShader(cubeMapping.PixelShader.Get(), 0, 0);
	//ID3D11ShaderResourceView* views[3] = 
	//{ 
	//	cubeMapping.envSRV.Get(),
	//	cubeMapping.specularSRV.Get(),
	//	cubeMapping.irradianceSRV.Get(),
	//};
	...
	//D3D::Get()->GetDeviceContext()->DrawIndexed(cubeMapping.CubeMesh->IndexCount, 0, 0);
}