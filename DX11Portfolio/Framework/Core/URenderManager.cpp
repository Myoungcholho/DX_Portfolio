...
void URenderManager::EnqueueProxies(vector<shared_ptr<URenderProxy>>&& proxies, vector<LightData>&& lights)
{
	// 쓰기 인덱스에 렌더링 리스트 저장하고 읽기 인덱스와 교체
	{
		...
		assert((writeIndex >= 0 && writeIndex < BUFFER_COUNT) && "writeIndex out of range");
		assert((readIndex >= 0 && readIndex < BUFFER_COUNT) && "readIndex out of range");
...
void URenderManager::RenderLoop()
{
	try
	{
		...
		while (!shouldExit.load(memory_order_acquire))
		{
			...
			// 대기하다가 렌더링이 준비되거나 종료 플래그가 true일때 탈출
			{
				...
				bool valid = (readIndex >= 0 && readIndex < BUFFER_COUNT);
				assert(valid && "readIndex out of range");
			...
			CContext* context = CContext::Get();
			assert(context != nullptr);

			CCamera* camera = context->GetCamera();
	...
	{
		// cout은 내부 버퍼 플러쉬 작업, 멀티 스레드 상황에서 적합하지 않다.
		OutputDebugStringA(("RenderLoop 예외: " + string(e.what()) + "\n").c_str());
	}
	catch (...)
	{
		OutputDebugStringA("RenderLoop 예외: 이유 불명 \n");
	}
}