...
ImageFilter::ImageFilter(ComPtr<ID3D11Device>& device,
    ...
    int height) 
{
    Initialize(device, context, InPixelShader, width, height);
}
    ...
    int height) 
{
    ...
    ThrowIfFailed(InPixelShader.CopyTo(pixelShader.GetAddressOf()));
...
{
    D3D11Utils::UpdateBuffer(device, context, constData, constBuffer);
}
...
void ImageFilter::Render(ComPtr<ID3D11DeviceContext>& context) const 
{
    ...
    context->OMSetRenderTargets(UINT(renderTargets.size()),renderTargets.data(), NULL);
    context->PSSetShader(pixelShader.Get(), 0, 0);
    context->PSSetShaderResources(0, UINT(shaderResources.size()),shaderResources.data());
